...
async function main() {
    ...
    try {
        ...
        const tx = await saleContract.enableEmergencyMode();
        const receipt = await tx.wait();
        console.log("Emergency mode enabled successfully! Transaction hash:", receipt.transactionHash);

    } catch (error) {
        console.error("Error enabling emergency mode:", error.message);
        if (error.data) {
