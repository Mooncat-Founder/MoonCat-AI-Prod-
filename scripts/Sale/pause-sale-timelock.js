// scripts/sale/Pause-sale-timelock.js
...
async function main() {
    ...
    // Get network info from hardhat
    const network = await hre.ethers.provider.getNetwork();
    const networkName = hre.network.name;
    console.log(`Network: ${networkName} (Chain ID: ${network.chainId})`);
    
    // Convert network name to uppercase and handle different formats
    const networkNameUpper = networkName.toUpperCase().replace(/-/g, '_');
    
    // Try both formats for environment variables
    const networkNameWithDash = networkNameUpper.replace(/_/g, '-');
    
    // Use network-prefixed environment variables
    const saleAddress = 
        process.env[`${networkNameUpper}_SALE_ADDRESS`] || 
        process.env[`${networkNameWithDash}_SALE_ADDRESS`];
    
    const timelockAddress = 
        process.env[`${networkNameUpper}_SALE_TIMELOCK_ADDRESS`] || 
        process.env[`${networkNameWithDash}_SALE_TIMELOCK_ADDRESS`];
    ...
    if (!saleAddress || !timelockAddress || !safeAddress) {
        // Debug output to show available environment variables
        ...
        if (envVars.length > 0) {
            envVars.forEach(key => {
                if (!key.includes('KEY')) { // Don't print any potential secret keys
    ...
    const safeConfig = loadSafeConfig(networkName);
    console.log(`Loaded configuration for environment: ${networkToEnv[networkName] || 'test'}`);
    ...
    try {
        ...
        await executeGnosisSafeTransaction(safeAddress, timelockAddress, scheduleData, safeSigners, safeConfig);
        ...
        await executeGnosisSafeTransaction(safeAddress, timelockAddress, executeData, safeSigners, safeConfig);
