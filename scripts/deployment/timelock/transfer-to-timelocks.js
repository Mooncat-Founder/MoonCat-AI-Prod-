...
// Select the appropriate keys file based on network
async function getSafeConfig() {
    const network = await ethers.provider.getNetwork();
    const networkName = network.name.toLowerCase();
    
    // Determine which key file to use based on network
    let keyFileName;
    if (networkName.includes('mainnet')) {
        keyFileName = 'keys.prod.json';
        console.log("Using production keys file (keys.prod.json)");
    } else {
        keyFileName = 'keys.test.json';
        console.log("Using test keys file (keys.test.json)");
    }
    ...
    if (!fs.existsSync(keyFilePath)) {
        throw new Error(`Keys file not found: ${keyFilePath}`);
    }
    ...
    try {
        ...
        throw new Error(`Failed to load keys file: ${error.message}`);
    }
}
...
async function main() {
    ...
    // Load safe configuration based on network
    const safeConfig = await getSafeConfig();
