// scripts/finalize-sale-through-timelock.js
...
async function main() {
    ...
    console.log("Starting finalize sale process through timelock...");
    ...
    try {
        ...
        const finalizeSaleData = saleInterface.encodeFunctionData("finalizeSale", []);

        // Parameters for the timelock operation
        const target = saleContractAddress; // The contract we want to finalize
        const value = 0; // No ETH being sent
        const data = finalizeSaleData; // The finalizeSale function call
        ...
        const salt = ethers.id(`finalize-sale-${Date.now()}`);
        ...
        saveOperation({
          ...
          target,
          value,
          data: finalizeSaleData,
          predecessor,
          salt,
          description: `Finalize sale operation for ${saleContractAddress}`,
          network,
          timestamp: Date.now()
        });
        ...
        if (operationExists) {
            ...
            if (isReady) {
                ...
                console.log('The sale should already be finalized.');
            }
        } else {
        ...
        console.error("Error during finalize sale through timelock:", error.message);
