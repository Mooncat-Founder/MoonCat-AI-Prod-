// scripts/withdraw-eth-through-timelock.js
...
async function main() {
    ...
    console.log("Starting withdraw ETH process through timelock...");
    ...
    try {
        ...
        // Get the current ETH balance and treasury wallet
        const ethBalance = await provider.getBalance(saleContractAddress);
        const treasuryWallet = await saleContract.treasuryWallet();
        
        console.log(`\nCurrent contract ETH balance: ${ethers.formatEther(ethBalance)} ETH`);
        console.log(`Treasury wallet: ${treasuryWallet}`);
        
        if (ethBalance === 0n) {
            console.log('There is no ETH to withdraw from the contract.');
            process.exit(0);
        }
        ...
        // Create the transaction data for withdrawing ETH
        const withdrawEthData = saleContract.interface.encodeFunctionData("withdrawETH", []);
        ...
        const data = withdrawEthData; // The withdrawETH function call
        ...
        const salt = ethers.id(`withdraw-eth-${Date.now()}`);
        ...
        saveOperation({
          ...
          predecessor,
          salt,
          description: `Withdraw ${ethers.formatEther(ethBalance)} ETH to treasury (${treasuryWallet})`,
          network,
          timestamp: Date.now()
        });
        ...
        if (operationExists) {
            ...
            if (isReady) {
                ...
                console.log('Executing ETH withdrawal through timelock...');
                ...
                if (receipt.status === 1) {
                    console.log('ETH withdrawn successfully through timelock!');
                    console.log('Transaction hash:', receipt.hash);
                    
                    // Check the new balance
                    const newBalance = await provider.getBalance(saleContractAddress);
                    console.log(`New contract ETH balance: ${ethers.formatEther(newBalance)} ETH`);
                } else {
                ...
                console.log('The ETH should already be withdrawn.');
            }
        } else {
        ...
        console.error("Error during ETH withdrawal through timelock:", error.message);
