// scripts/withdraw-usdt-through-timelock.js
...
// Token Sale ABI for balance check
...
// ERC20 ABI for USDT balance check
const ERC20_ABI = [
...
async function main() {
    ...
    console.log("Starting withdraw USDT process through timelock...");
    ...
    try {
        ...
        // Get the USDT token address and treasury wallet
        ...
        // Get the current USDT balance and decimals
        const usdtBalance = await usdtContract.balanceOf(saleContractAddress);
        const usdtDecimals = await usdtContract.decimals();
        
        // Format the USDT balance for display
        const formattedBalance = ethers.formatUnits(usdtBalance, usdtDecimals);
        
        console.log(`\nUSDT Token Address: ${usdtTokenAddress}`);
        console.log(`Treasury Wallet: ${treasuryWallet}`);
        console.log(`Current contract USDT balance: ${formattedBalance} USDT`);
        
        if (usdtBalance === 0n) {
            console.log('There is no USDT to withdraw from the contract.');
            process.exit(0);
        }
        ...
        // Create the transaction data for withdrawing USDT
        const withdrawUsdtData = saleContract.interface.encodeFunctionData("withdrawUSDT", []);
        ...
        const data = withdrawUsdtData; // The withdrawUSDT function call
        ...
        const salt = ethers.id(`withdraw-usdt-${Date.now()}`);
        ...
        saveOperation({
          ...
          target,
          value,
          data: withdrawUsdtData,
          predecessor,
          salt,
          description: `Withdraw ${formattedBalance} USDT to treasury (${treasuryWallet})`,
          network,
          timestamp: Date.now()
        });
        ...
        if (operationExists) {
            ...
            if (isReady) {
                ...
                console.log('Executing USDT withdrawal through timelock...');
                ...
                if (receipt.status === 1) {
                    console.log('USDT withdrawn successfully through timelock!');
                    console.log('Transaction hash:', receipt.hash);
                    
                    // Check the new balance
                    const newBalance = await usdtContract.balanceOf(saleContractAddress);
                    const formattedNewBalance = ethers.formatUnits(newBalance, usdtDecimals);
                    console.log(`New contract USDT balance: ${formattedNewBalance} USDT`);
                } else {
                ...
                console.log('The USDT should already be withdrawn.');
            }
        } else {
        ...
        console.error("Error during USDT withdrawal through timelock:", error.message);
