// scripts/enable-emergency-mode-through-timelock.js
...
async function main() {
    ...
    console.log("Starting enable emergency mode process through timelock...");
    ...
    try {
        ...
        console.log(`\nCurrent emergency mode status: ${isEmergencyMode ? 'ENABLED' : 'DISABLED'}`);
        
        if (isEmergencyMode) {
            console.log('\nWARNING: Emergency mode is already enabled.');
            console.log('You can still proceed, but this operation will have no effect.');
        } else {
            console.log('\nWARNING: Enabling emergency mode will:');
            console.log('1. Disable all new token purchases');
            console.log('2. Allow users to withdraw their contributions');
            console.log('3. Put the sale in an emergency state');
            console.log('This is intended for emergencies only and should be carefully considered.');
        }
        ...
        // Create the transaction data for enabling emergency mode
        const enableEmergencyData = saleContract.interface.encodeFunctionData("enableEmergencyMode", []);
        ...
        const data = enableEmergencyData; // The enableEmergencyMode function call
        ...
        const salt = ethers.id(`enable-emergency-${Date.now()}`);
        ...
        saveOperation({
          ...
          target,
          value,
          data: enableEmergencyData,
          predecessor,
          salt,
          description: `Enable emergency mode on sale contract ${saleContractAddress}`,
          network,
          timestamp: Date.now()
        });
        ...
        if (operationExists) {
            ...
            if (isReady) {
                ...
                console.log('Executing enable emergency mode through timelock...');
                ...
                if (receipt.status === 1) {
                    console.log('Emergency mode enabled successfully through timelock!');
                    console.log('Transaction hash:', receipt.hash);
                    
                    // Check the new emergency mode status
                    const newEmergencyMode = await saleContract.emergencyMode();
                    console.log(`New emergency mode status: ${newEmergencyMode ? 'ENABLED' : 'DISABLED'}`);
                    
                    if (newEmergencyMode) {
                        console.log('Emergency mode is now enabled. Users can withdraw their contributions.');
                    } else {
                        console.log('Warning: Emergency mode is still disabled after the operation. This should not happen.');
                    }
                } else {
                ...
                console.log('Emergency mode should already be enabled.');
            }
        } else {
        ...
        console.error("Error during enable emergency mode through timelock:", error.message);
