// scripts/disable-emergency-mode-through-timelock.js
...
async function main() {
    ...
    console.log("Starting disable emergency mode process through timelock...");
    ...
    try {
        ...
        if (!isEmergencyMode) {
            console.log('\nWARNING: Emergency mode is already disabled.');
            console.log('You can still proceed, but this operation will have no effect.');
        } else {
            console.log('\nWARNING: Disabling emergency mode will:');
            console.log('1. Re-enable token purchases');
            console.log('2. Stop users from emergency withdrawals');
            console.log('3. Return the sale to normal operation');
            console.log('This should only be done if the emergency situation has been resolved.');
        }
        ...
        // Create the transaction data for disabling emergency mode
        const disableEmergencyData = saleContract.interface.encodeFunctionData("disableEmergencyMode", []);
        ...
        const data = disableEmergencyData; // The disableEmergencyMode function call
        ...
        const salt = ethers.id(`disable-emergency-${Date.now()}`);
        ...
        saveOperation({
          ...
          target,
          value,
          data: disableEmergencyData,
          predecessor,
          salt,
          description: `Disable emergency mode on sale contract ${saleContractAddress}`,
          network,
          timestamp: Date.now()
        });
        ...
        if (operationExists) {
            ...
            if (isReady) {
                ...
                console.log('Executing disable emergency mode through timelock...');
                ...
                if (receipt.status === 1) {
                    console.log('Emergency mode disabled successfully through timelock!');
                    ...
                    if (!newEmergencyMode) {
                        console.log('Emergency mode is now disabled. Normal sale operations have resumed.');
                    } else {
                        console.log('Warning: Emergency mode is still enabled after the operation. This should not happen.');
                    }
                } else {
                ...
                console.log('Emergency mode should already be disabled.');
            }
        } else {
        ...
        console.error("Error during disable emergency mode through timelock:", error.message);
