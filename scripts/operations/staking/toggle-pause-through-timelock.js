// scripts/toggle-pause-through-timelock.js
...
// Staking contract ABI for pause/unpause functions
const STAKING_ABI = [
...
async function main() {
  try {
    ...
    console.log("Starting pause/unpause process through timelock...");
    ...
    console.log('\nCurrent contract status:', isPaused ? 'PAUSED' : 'ACTIVE');
    ...
    // Ask user to toggle pause state
    const action = isPaused ? 'unpause' : 'pause';
    console.log(`\nThis script will ${action} the contract.`);
    const confirmAction = await prompt(`Confirm ${action} contract? (y/n): `);
    ...
    // Create the function call to toggle pause
    const stakingInterface = new ethers.Interface(STAKING_ABI);
    const funcName = isPaused ? "unpause" : "pause";
    ...
    const salt = ethers.id(`${action}-contract-${Date.now()}`);
    ...
    saveOperation({
      ...
      predecessor,
      salt,
      description: `${action.charAt(0).toUpperCase() + action.slice(1)} staking contract`,
      network,
      timestamp: Date.now()
    });
    ...
    if (operationExists) {
      ...
      if (isReady) {
        ...
        console.log(`Executing ${action} contract through timelock...`);
        ...
        if (receipt.status === 1) {
          console.log(`Contract successfully ${action}d through timelock!`);
          console.log('Transaction hash:', receipt.hash);
          
          // Verify the updated status
          const newPauseStatus = await stakingContract.paused();
          console.log('\nNew contract status:', newPauseStatus ? 'PAUSED' : 'ACTIVE');
        } else {
        ...
        console.log('Contract pause status should already be updated.');
      }
    } else {
    ...
    console.log('Description:', `${action.charAt(0).toUpperCase() + action.slice(1)} staking contract`);
    
  } catch (error) {
    console.error("Error during contract pause/unpause:", error.message);
