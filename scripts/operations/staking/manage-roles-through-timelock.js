// scripts/manage-roles-through-timelock.js
...
async function main() {
  try {
    ...
    console.log("Starting role management process through timelock...");
    ...
    const stakingContract = new ethers.Contract(stakingContractAddress, ACCESS_CONTROL_ABI, provider);
    
    // Get role constants from contract
    const DEFAULT_ADMIN_ROLE = await stakingContract.DEFAULT_ADMIN_ROLE();
    const GOVERNOR_ROLE = await stakingContract.GOVERNOR_ROLE();
    ...
    // Ask which role to manage
    console.log('\nWhich role would you like to manage?');
    console.log('1: DEFAULT_ADMIN_ROLE');
    console.log('2: GOVERNOR_ROLE');
    console.log('3: PAUSER_ROLE');
    
    const roleChoice = await prompt('Enter choice (1, 2, or 3): ');
    ...
    // Display current role members
    try {
      const memberCount = await stakingContract.getRoleMemberCount(role);
      console.log(`\nCurrent ${roleName} members (${memberCount}):`);
      
      for (let i = 0; i < Number(memberCount); i++) {
        const member = await stakingContract.getRoleMember(role, i);
        console.log(`${i + 1}: ${member}`);
      }
    } catch (error) {
      console.log(`Could not retrieve role members: ${error.message}`);
    }
    
    // Ask action for the role
    console.log('\nWhat action would you like to perform?');
    console.log('1: Grant role to a new address');
    console.log('2: Revoke role from an existing address');
    
    const actionChoice = await prompt('Enter choice (1 or 2): ');
    ...
    if (actionChoice === '1') {
      action = 'grant';
      targetAddress = await prompt('Enter address to grant the role to: ');
    } else if (actionChoice === '2') {
      action = 'revoke';
      targetAddress = await prompt('Enter address to revoke the role from: ');
    } else {
    ...
    const hasRole = await stakingContract.hasRole(role, targetAddress);
    
    if (action === 'grant' && hasRole) {
      console.log(`\nWarning: Address ${targetAddress} already has the ${roleName} role.`);
    ...
    } else if (action === 'revoke' && !hasRole) {
      console.log(`\nWarning: Address ${targetAddress} does not have the ${roleName} role.`);
      const confirm = await prompt('Continue anyway? (y/n): ');
      if (confirm.toLowerCase() !== 'y') {
        console.log('Operation cancelled.');
        rl.close();
    ...
    console.log(`\nYou are about to ${action} the ${roleName} role ${action === 'grant' ? 'to' : 'from'} ${targetAddress}`);
    const finalConfirm = await prompt('Confirm this action? (y/n): ');
    ...
    const stakingInterface = new ethers.Interface(ACCESS_CONTROL_ABI);
    const funcName = action === 'grant' ? "grantRole" : "revokeRole";
    const callData = stakingInterface.encodeFunctionData(funcName, [role, targetAddress]);
    ...
    const salt = ethers.id(`${action}-${roleName}-${targetAddress}-${Date.now()}`);
    ...
    saveOperation({
      ...
      predecessor,
      salt,
      description: `${action.charAt(0).toUpperCase() + action.slice(1)} ${roleName} ${action === 'grant' ? 'to' : 'from'} ${targetAddress}`,
      network,
      timestamp: Date.now()
    });
    ...
    if (operationExists) {
      ...
      if (isReady) {
        ...
        console.log(`Executing ${action} role through timelock...`);
        ...
        if (receipt.status === 1) {
          console.log(`Role ${action === 'grant' ? 'granted' : 'revoked'} successfully through timelock!`);
          console.log('Transaction hash:', receipt.hash);
          
          // Verify the role status
          const hasRoleNow = await stakingContract.hasRole(role, targetAddress);
          console.log(`\nAddress ${targetAddress} ${hasRoleNow ? 'has' : 'does not have'} the ${roleName} role.`);
        } else {
        ...
        console.log('The role should already be updated.');
      }
    } else {
    ...
    console.log('Description:', `${action.charAt(0).toUpperCase() + action.slice(1)} ${roleName} ${action === 'grant' ? 'to' : 'from'} ${targetAddress}`);
    
  } catch (error) {
    console.error("Error during role management:", error.message);
