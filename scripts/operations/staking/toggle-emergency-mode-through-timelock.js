// scripts/toggle-emergency-mode-through-timelock.js
...
// Staking contract ABI for emergency mode functions
const STAKING_ABI = [
...
async function main() {
  try {
    ...
    console.log("Starting emergency mode toggle process through timelock...");
    ...
    console.log('\nCurrent emergency mode status:', emergencyModeActive ? 'ENABLED' : 'DISABLED');
    ...
    // Ask user to toggle emergency mode state
    const action = emergencyModeActive ? 'disable' : 'enable';
    console.log(`\nThis script will ${action} emergency mode.`);
    const confirmAction = await prompt(`Confirm ${action} emergency mode? (y/n): `);
    ...
    // Create the function call to toggle emergency mode
    const stakingInterface = new ethers.Interface(STAKING_ABI);
    const funcName = emergencyModeActive ? "disableEmergencyMode" : "enableEmergencyMode";
    ...
    const salt = ethers.id(`${action}-emergency-mode-${Date.now()}`);
    ...
    saveOperation({
      ...
      predecessor,
      salt,
      description: `${action.charAt(0).toUpperCase() + action.slice(1)} emergency mode`,
      network,
      timestamp: Date.now()
    });
    ...
    if (operationExists) {
      ...
      if (isReady) {
        ...
        console.log(`Executing ${action} emergency mode through timelock...`);
        ...
        if (receipt.status === 1) {
          console.log(`Emergency mode successfully ${action}d through timelock!`);
          console.log('Transaction hash:', receipt.hash);
          
          // Verify the updated status
          const newEmergencyModeStatus = await stakingContract.emergencyMode();
          console.log('\nNew emergency mode status:', newEmergencyModeStatus ? 'ENABLED' : 'DISABLED');
        } else {
        ...
        console.log('Emergency mode should already be updated.');
      }
    } else {
    ...
    console.log('Description:', `${action.charAt(0).toUpperCase() + action.slice(1)} emergency mode`);
    
  } catch (error) {
    console.error("Error during emergency mode toggle:", error.message);
